...
Gas level has the highest priority.

If the gas level exceeds a threshold:

ventilation is activated

the system enters an emergency state

Temperature

Temperature control for storage conditions.

If the temperature goes outside the acceptable range:

cooling or heating can be activated

Humidity

Humidity control.

If humidity exceeds acceptable limits:

the system may activate dehumidification or humidification

Light

Light has the lowest priority.

It is used to monitor storage conditions.

Workflow

System operation cycle:

ESP32 reads data from sensors

Data is processed inside the microcontroller

ESP32 sends telemetry via MQTT

Node-RED receives the messages

Node-RED writes the data to InfluxDB
...
adding ventilation and actuator systems

developing a fully autonomous storage monitoring system
